...
class InterceptionManager<Args extends any[], ReturnType, ContextType = Record<string, any>> {
  protected interceptions: Array<Interceptor<Args, ReturnType, ContextType>>;
  private interceptionKeySet: Set<keyof Interceptor<Args, ReturnType, ContextType>>;
...
abstract class Hook<Args extends any[], ReturnType, ContextType = Record<string, any>>
  implements SyncBaseHookType<Args, ReturnType, ContextType>
{
  ...
  constructor() {
    this.taps = [];
    this.interceptions = new InterceptionManager<Args, ReturnType, ContextType>();
  }
  ...
  public tap(name: string, callback: (...args: Args) => ReturnType): Tap<Args, ReturnType, ContextType>;
...
export class SyncHook<Args extends any[], ContextType = Record<string, any>> extends Hook<Args, void, ContextType> {
...
export class SyncBailHook<Args extends any[], ReturnType, ContextType = Record<string, any>> extends Hook<
  Args,
...
export class SyncWaterfallHook<Args extends any[], ContextType = Record<string, any>> extends Hook<
  Args,
  Args[0],
  ContextType
> {
...
export class SyncLoopHook<Args extends any[], ContextType = Record<string, any>> extends Hook<Args, void, ContextType> {
  public call(...args: Args) {
    let finished = false;
    const ctx: ContextType = {} as any;
...
export class AsyncParallelHook<Args extends any[], ContextType = Record<string, any>> extends Hook<
  Args,
  Promise<void>,
  ContextType
> {
...
export class AsyncParallelBailHook<Args extends any[], ReturnType, ContextType = Record<string, any>> extends Hook<
  Args,
  Promise<ReturnType>,
  ContextType
> {
  public async call(...args: Args): Promise<ReturnType> {
    ...
    try {
      const rtn = await Promise.race(this.taps.map((tap) => callTap(tap, args, ctx)));
...
export class AsyncSeriesHook<Args extends any[], ContextType = Record<string, any>> extends Hook<
  Args,
  Promise<void>,
  ContextType
> {
...
export class AsyncSeriesBailHook<Args extends any[], ReturnType, ContextType = Record<string, any>> extends Hook<
  Args,
  Promise<ReturnType | undefined | null>,
  ContextType
> {
...
export class AsyncSeriesWaterfallHook<Args extends any[], ContextType = Record<string, any>> extends Hook<
  Args,
  Promise<Args[0]>,
  ContextType
> {
  public async call(...args: Args): Promise<Args[0]> {
    ...
    try {
      for (let tapIndex = 0; tapIndex < this.taps.length; tapIndex += 1) {
        const tapValue = await callTap(this.taps[tapIndex], [rtn, ...rest] as any, ctx);
        if (tapValue !== undefined) {
          rtn = tapValue;
        }
      }
...
export class AsyncSeriesLoopHook<Args extends any[], ContextType = Record<string, any>> extends Hook<
  Args,
  Promise<void>,
  ContextType
> {
