...
export class HttpFetch extends HttpAdapter {
  ...
  handle<T>(request: HttpRequest<SafeAny>): Observable<T> {
    return new Observable((observer) => {
      const aborter = new AbortController();
      this.doRequest<T>(request, aborter.signal, observer).then(noop, (error) =>
   ...
   * @private
   */
  private async doRequest<T>(request: HttpRequest<T>, signal: AbortSignal, observer: T): Promise<void> {
    let abortable = true;
    observer.next({ type: HttpEventType.Sent });

    const init = this.createRequestInit(request);

    let selector: (response: Response) => ObservableInput<SafeAny>;
