...
const stringify = (value: ParameterValue) => (value === null || value === undefined ? '' : `${value}`);
...
export type HttpParamsValue = string | Record<string, ParameterValue | ReadonlyArray<ParameterValue>>;
      ...
      source
        ...
        .forEach((param: string) => {
          const [key, value] = param
            .split(/=(.*)/, 2)
            .map((v, i) => (v === undefined ? '' : i === 0 ? this.encoder.decodeKey(v) : this.encoder.decodeValue(v)));
  ...
  toString(): string {
    return this.keys()
      .map((key) =>
        this.map
          .get(key)!
          .map((value) => this.encoder.encodeKey(key) + '=' + this.encoder.encodeValue(value))
          .join('&'),
      )
      .filter((param) => param !== '')
      .join('&');
