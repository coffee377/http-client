...
import {OptsHook, PrefixHook, UriHook} from './hooks';
...
export class HttpClient {
  ...
  request<R = any>(urlOrOptions: string | HttpClientOptions, options: HttpClientOptions): Promise<R> {
    const transform = this.transformHttpClientOptions(urlOrOptions, options);
    ...
    const url = this.hooks.url.call(opts.url, opts);
